...
_accountDelta方法中的delta参数 的正负是从PoolManager的角度来评判的： 负数 用户要支付的。表示PM的余额少了，即为表示用户从PoolManager提取了代币,或者声称要给，还没给，所以后续用户要还上。正 用户可以提取的，表示PM的余额多了，表示用户向PoolManager发送了代币，或者poolManager欠了用户的钱，用户还没有拿走，后续用户可以提取其他代币。
