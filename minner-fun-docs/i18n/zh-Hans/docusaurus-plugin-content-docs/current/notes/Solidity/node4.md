...
#### 获取合约的创建字节码
```solidity
bytes memory bytecode = type(DeployWithCreate2).creationCode; // 获取合约的创建字节码
```
![](./类型信息.png)
...
contract Factory {
    event Deployed(address addr, string how);

    // ---------- CREATE ----------
    function deployWithCreate(address _owner) external returns (address addr) {
        addr = address(new Deployed(_owner)); // 普通 CREATE
        emit Deployed(addr, "CREATE");
    }

    // ---------- CREATE2 ----------
    function deployWithCreate2(bytes32 salt, address _owner) external returns (address addr) {
        addr = address(new Deployed{salt: salt}(_owner)); // CREATE2
        emit Deployed(addr, "CREATE2");
    }

    // 生成 CREATE2 所需 initcode（= creationCode + constructor args）
    function getBytecode(address _owner) public pure returns (bytes memory) {
        return abi.encodePacked(type(Deployed).creationCode, abi.encode(_owner));
    }

    // 依据 EIP-1014 公式计算 CREATE2 的地址
    function getCreate2Address(uint256 salt, address _owner) public view returns (address) {
...
describe("CREATE vs CREATE2", function () {
  let factory, owner, other;
  ...
  it("predict address for CREATE (using ethers.getCreateAddress)", async () => {
    // 合约账户的 CREATE nonce：从 1 开始计数（首次 create 用 nonce=1）
    const predicted = ethers.getCreateAddress({
      from: factory.target, // v6: 合约地址
      nonce: 1n,
    });

    const tx = await factory.deployWithCreate(owner.address);
    const rc = await tx.wait();
    const deployed = rc.logs.find((l) => l.fragment?.name === "Deployed").args.addr;

    expect(deployed).to.equal(predicted);
  });

  it("predict address for CREATE2 (on-chain helper & off-chain formula)", async () => {
    const salt = 12345n;

    // 方式A：链上 helper（与部署一致）
    const predictedOnChain = await factory.getCreate2Address(
    ...
    const predictedOffChain = ethers.getCreate2Address(
      factory.target,
      ethers.toBeHex(salt, 32),
      initCodeHash
    );

    expect(predictedOffChain).to.equal(predictedOnChain);

    // 真正部署
    const tx = await factory.deployWithCreate2(ethers.toBeHex(salt, 32), owner.address);
    const rc = await tx.wait();
    const deployed = rc.logs.find((l) => l.fragment?.name === "Deployed").args.addr;

    expect(deployed).to.equal(predictedOnChain);
  });

  it("changing salt or constructor args changes CREATE2 address", async () => {
    const salt1 = 1n, salt2 = 2n;

    const addr1 = await factory.getCreate2Address(ethers.toBeHex(salt1, 32), owner.address);
    const addr2 = await factory.getCreate2Address(ethers.toBeHex(salt2, 32), owner.address);
    const addr3 = await factory.getCreate2Address(ethers.toBeHex(salt1, 32), other.address);

    expect(addr1).to.not.equal(addr2); // salt 不同 -> 地址不同
    expect(addr1).to.not.equal(addr3); // 构造参数不同 -> 地址不同
  });
});
