...
### 🧩 BDD 风格的三段式结构
BDD（Behavior Driven Development，行为驱动开发）的写作风格，
Given：前置条件（测试场景的初始状态） 比如 “系统里已有 3 个 admin，cedric 还不是 admin”
When：执行的动作（触发的行为） 比如 “调用 addAdmin(cedric.address)”
Then：期望的结果（断言/验证） 比如 “现在有 4 个 admin，并且 cedric 已经是 admin 了”
```js
