...
单态化：编译包含泛型的 Rust 代码时，编译器会执行单态化。这意味着它会为每个具体类型生成特定的泛型代码版本。例如，如果您使用 `Vec<i32>` 和 `Vec<String>`，编译器会为 `i32` 和 `String` 分别生成专门的代码。此过程确保在 Rust 中使用泛型不会像手动编写专门代码那样带来运行时性能损失。
